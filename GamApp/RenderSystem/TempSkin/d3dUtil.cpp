	...
	std::vector<IDirect3DTexture9*>& texs)
{
	...
	HR(D3DXLoadMeshFromX(filename.c_str(), D3DXMESH_SYSTEMMEM, RENDERDEVICE::Instance().g_pD3DDevice,
	...
	HR(meshSys->CloneMesh(D3DXMESH_SYSTEMMEM, 
		elements, RENDERDEVICE::Instance().g_pD3DDevice, &temp));
	ReleaseCOM(meshSys);
	meshSys = temp;
	...
	if( mtrlBuffer != 0 && numMtrls != 0 )
	{
		...
		for(DWORD i = 0; i < numMtrls; ++i)
		{
			...
			if( d3dxmtrls[i].pTextureFilename != 0 )
			{
				...
				HR(D3DXCreateTextureFromFile(RENDERDEVICE::Instance().g_pD3DDevice, texFN, &tex));
